...
>(
  ...
  const itemRefs = useMemo<React.RefObject<HTMLLIElement>[]>(
  ...
  // Focus the first item or the selected item when the menu is opened
  useEffect(() => {
    if (isOpen) {
      const selectedOption = itemRefs.findIndex(
        (ref) => ref.current?.ariaLabel === selected
      );
  ...
  useEffect(() => {
    ...
    const handleEveryClick = (event: MouseEvent): void => {
      ...
      setTimeout(() => {
        ...
        if (event.target.closest('[role="listbox"]') instanceof Element) {
          return;
        }
  ...
  const itemListener = (e: React.KeyboardEvent<HTMLLIElement>): void => {
